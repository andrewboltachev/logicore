...
urlpatterns = (
    [
        re_path(
            r"(?P<prefix>@react-refresh)", main_views.react_dev_data
        ),  # last one. handle all 404
        re_path(
            r"(?P<prefix>src|@vite|node_modules)/(?P<path>.*)",
            main_views.react_dev_data,
        ),  # last one. handle all 404
        path(
            "robots.txt",
        ...
        # re_path('^embellished-api/matchToFunnel$', main_views.embellishedMatchToFunnel),
        re_path(
            "^logicore-api/(?P<subtype>.+)/(?P<action>.+)/$", main_views.logicore_api
        ),
    ]
    + main_urls
...
if settings.FRONTEND_DEV_MODE:
    ...
    urlpatterns = (
        static(
            "/react-static/",
            document_root=settings.BASE_DIR / "frontend" / "build" / "react-static",
        )
        + urlpatterns
    )
